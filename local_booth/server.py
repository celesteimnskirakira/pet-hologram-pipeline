...
go.onclick=async()=>{err.textContent='';if(!image.files[0]){err.textContent='请先选择照片';return}go.disabled=true;const data=new FormData();data.append('image',image.files[0]);data.append('pet_name',name.value);data.append('pose',pose.value);const r=await fetch('/api/enqueue?k='+encodeURIComponent(key),{method:'POST',body:data});const d=await r.json();if(!r.ok){err.textContent=d.error||'提交失败';go.disabled=false;return}id=d.job_id;form.hidden=true;status.hidden=false;poll()};
